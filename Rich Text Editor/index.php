...
$conn=mysqli_connect("localhost", "root", "", "richtexteditordb");

if(isset($_POST['saveNoteName'])) {
	$richTextContent = mysqli_real_escape_string($conn, $_POST['richTextField']);
	
	$conn->query("INSERT INTO noteTable (note_content) VALUES ('$richTextContent')");
	
	if($conn->affected_rows != 1) {
		echo '<script type="text/javascript"> alert("something went wrong"); </script>';
	} else {
		echo '<script type="text/javascript"> alert("NOTE RECORD SUCCESSFUL"); </script>';
	}
}
...
<iframe name="richTextField" style="width:80%;height: 80%;margin:auto;display: block;" form="noteForm">

</iframe>
<input type="submit" id="saveNote" name="saveNoteName">SAVE TO DATABASE</button>
</form>
